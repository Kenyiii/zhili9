...
const EditorView: React.FC<EditorViewProps> = ({ onImageGenerated }) => {
  ...
  const runFullPipeline = async () => {
    ...
    try {
      ...
      // Fix potential unknown type issue by explicitly typing the result of processImage
      const originalUrl: string = await GeminiService.processImage(sourceImage, getPromptForType('地毯原图', targetW, targetH), apiRatio);
      setResults((prev: Record<string, string | null>) => ({ ...prev, '地毯原图': originalUrl }));

      setPipelineProgress(2);
      // 主图_01
      const main01Url: string = await GeminiService.processImage(originalUrl, getPromptForType('主图_01', targetW, targetH), apiRatio);
      setResults((prev: Record<string, string | null>) => ({ ...prev, '主图_01': main01Url }));

      setPipelineProgress(3);
      // 主图_05 (Material details)
      const main05Url: string = await GeminiService.processImage(originalUrl, getPromptForType('主图_05', targetW, targetH), '1:1');
      setResults((prev: Record<string, string | null>) => ({ ...prev, '主图_05': main05Url }));

      setPipelineProgress(4);
      
      // Fix: Explicitly type return of local async helper to ensure Promise.all result is known as string[]
      ...
      // Fix: Cast Promise.all result or use an intermediate typed variable to avoid 'unknown' destructuring
      const resultsArray: string[] = await Promise.all([
        fetchWithDelay('主图_02', originalUrl, 0),
        fetchWithDelay('主图_03', originalUrl, 1000),
        fetchWithDelay('主图_04', originalUrl, 2000),
      ]);

      const [s2, s3, s4] = resultsArray;
